

const menuData ={
    "drinks": [
        {
            "name": "Espresso",
            "type": "Coffee",
            "category": "Hot",
            "price": "1.80/2.40/2.90"
        },
        {
            "name": "Espresso Macchiato",
            "type": "Coffee",
            "category": "Hot",
            "price": "2.50"
        },
        {
            "name": "Americano",
            "type": "Coffee",
            "category": "Hot",
            "price": "2.30/2.80"
        },
        {
            "name": "Cappuccino",
            "type": "Coffee",
            "category": "Hot",
            "price": "2.80/3.40/3.90"
        },
        {
            "name": "Flat White",
            "type": "Coffee",
            "category": "Hot",
            "price": "3.00"
        },
        {
            "name": "Latte",
            "type": "Coffee",
            "category": "Hot",
            "price": "3.00"
        },
        {
            "name": "Latte Caramel",
            "type": "Coffee",
            "category": "Hot",
            "price": "3.30"
        },
        {
            "name": "Φίλτρου",
            "type": "Coffee",
            "category": "Hot",
            "price": "3.00"
        },
        {
            "name": "Mocha",
            "type": "Coffee",
            "category": "Hot",
            "price": "3.50"
        },
        {
            "name": "Ελληνικός",
            "type": "Coffee",
            "category": "Hot",
            "price": "1.80/2.30/2.80"
        },
        {
            "name": "Nescafe",
            "type": "Coffee",
            "category": "Hot",
            "price": "2.50"
        },
        {
            "name": "Freddo Espresso",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.00/3.50"
        },
        {
            "name": "Freddo Cappuccino",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.00/3.50"
        },
        {
            "name": "Iced Latte",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.00"
        },
        {
            "name": "Iced Latte Caramel",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.50"
        },
        {
            "name": "Freddo Caramel",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.50"
        },
        {
            "name": "Iced Mochaccino",
            "type": "Coffee",
            "category": "Cold",
            "price": "3.50"
        },
        {
            "name": "Frappe",
            "type": "Coffee",
            "category": "Cold",
            "price": "2.50"
        },
        {
            "name": "English Breakfast",
            "type": "Tea",
            "category": "Black",
            "price": "3.50",
            "img" : "../images/tea.png"
        },
        {
            "name": "Early Grey Blue Flower",
            "type": "Tea",
            "category": "Black",
            "price": "3.50"
        },
        {
            "name": "Chai Tea",
            "type": "Tea",
            "category": "Black",
            "price": "2.50"
        },
        {
            "name": "Darjeeling Margaret's Hope",
            "type": "Tea",
            "category": "Black",
            "price": "3.50"
        },
        {
            "name": "Vanilla Tea",
            "type": "Tea",
            "category": "Black",
            "price": "3.50"
        }, {
            "name": "Japan Sencha",
            "type": "Tea",
            "category": "Green",
            "price": "3.50"
        },
        {
            "name": "Chia Gunpowder",
            "type": "Tea",
            "category": "Green",
            "price": "3.50"
        },
        {
            "name": "China Jasmine",
            "type": "Tea",
            "category": "Green",
            "price": "3.50"
        },
        {
            "name": "Fruit Melange 'Christ'",
            "type": "Tea",
            "category": "Red",
            "price": "3.50"
        },
        {
            "name": "Rooitea 'Capetown'",
            "type": "Tea",
            "category": "Red",
            "price": "3.50"
        },
        {
            "name": "Cream Caramel",
            "type": "Tea",
            "category": "Red",
            "price": "3.50"
        },
        {
            "name": "Fireplace",
            "type": "Tea",
            "category": "Red",
            "price": "3.50"
        },
        {
            "name": "Camillo",
            "type": "Tea",
            "category": "Mixed",
            "price": "3.50"
        },
        {
            "name": "Apple Ginger",
            "type": "Tea",
            "category": "Mixed",
            "price": "3.50"
        },
        {
            "name": "Κλασικη ζεστή/κρύα",
            "type": "Chocolate",
            "category": "",
            "price": "3.50",
            "type_info" : "σιρόπι +0.40 , σαντιγί +0.40"

        },
        {
            "name": "Λευκή Σοκολάτα",
            "type": "Chocolate",
            "category": "",
            "price": "3.50",
            "type_info" : "σιρόπι +0.40 , σαντιγί +0.40"
        },
        {
            "name": "Γρανίτα σοκολάτας",
            "type": "Chocolate",
            "category": "",
            "price": "3.50",
            "type_info" : "σιρόπι +0.40 , σαντιγί +0.40"
        },
        {
            "name": "Γεύσεις",
            "type": "Chocolate",
            "category": "",
            "discription" : "0% Ζάχαρη / Πραλίνα Φουντουκιού / Φράουλα / Καραμέλα με θαλασσινό αλάτι / Bueno / Bitter",
            "price": "3.50",
            "type_info" : "σιρόπι +0.40 , σαντιγί +0.40"
        },
        {
            "name": "Vanilla Caramel",
            "type": "Special Drinks",
            "category": "Hot",
            "price": "4.50",
        },
        {
            "name": "Vanilla Biscuit",
            "type": "Special Drinks",
            "category": "Hot",
            "price": "4.50",
        },
        {
            "name": "Mocha hazelnut",
            "type": "Special Drinks",
            "category": "Hot",
            "price": "4.50",
        },
        {
            "name": "Vanilla Ice",
            "type": "Special Drinks",
            "category": "Cold",
            "price": "4.20",
        },
        {
            "name": "Vanilla Ice Caramel",
            "type": "Special Drinks",
            "category": "Cold",
            "price": "4.50",
        },
        {
            "name": "Full Chocolate Ice",
            "type": "Special Drinks",
            "category": "Cold",
            "price": "4.50",
        },
        {
            "name": "Παγολεμονάδα",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.50",
        },
        {
            "name": "Παγοπορτοκαλάδα",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.50",
        },
        {
            "name": "Κανελάδα ζεστή/κρύα",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.50",
        },
        {
            "name": "Σπιτικές λεμονάδες B Fresh",
            "type": "Soft Drinks",
            "discription" : "Μαστίχα Χίου/Ginger/Grapefruit/Βύσσινο",
            "category": "",
            "price": "4",
        },
        {
            "name": "Φυσικός χυμός πορτοκάλι",
            "type": "Soft Drinks",
            "category": "",
            "price": "4",
        },
        {
            "name": "Φυσικός χυμός ανάμεικτος",
            "type": "Soft Drinks",
            "category": "",
            "price": "4.5",
        },
        {
            "name": "Φυσικός χυμός μπανάνας",
            "type": "Soft Drinks",
            "discription" : "γάλα αμυγδάλου +0.50",
            "category": "",
            "price": "4",
        },
        {
            "name": "Coca Cola / zero",
            "type": "Soft Drinks",
            "category": "",
            "price": "4.20",
        },
        {
            "name": "Fanta blue/lemon/orange",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Sprite",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Schweppes soda/grapefruit",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Amita motion",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Amita supermix",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Amita cherry",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.20",
        },
        {
            "name": "Ice tea ροδάκινο/λεμόνι",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.80",
        },
        {
            "name": "Monster πράσινο/άσπρο/μπλε",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.50",
        },
        {
            "name": "Ανθρακούχο νερό ζαρός",
            "type": "Soft Drinks",
            "category": "",
            "price": "3.50",
        },
        {
            "name": "Νερό ζαρός 0.5lt",
            "type": "Soft Drinks",
            "category": "",
            "price": "0.50",
        },
        {
            "name": "Νερό ζαρός 1lt",
            "type": "Soft Drinks",
            "category": "",
            "price": "1.00",
        }
        
    ],
    "food": [
        {
            "name": "Ομελέτα αλλαντικών",
            "type": "Brunch",
            "category": "Ομελέτες",
            "price": "6.00",
            "discription" : "μπέικον, γαλοπούλα, λουκάνικο Φρανγκφουρτης, ψωμί ολικής, μαγιονέζα, ρόκα, ντοματίνια, βαλσάμικο",
            "type_info" : "σερβίρεται εώς τις 15:30",
            "category_info" : "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες"
        },
        {
            "name": "Ομελέτα τυριών",
            "type": "Brunch",
            "category": "Ομελέτες",
            "price": "6.00",
            "discription" : "γραβιέρα, gouda, edam, φέτα, κρέμα αβοκάντο, ψωμί ολικής, ρόκα, ντοματίνια, βαλσάμικο",
            "type_info" : "σερβίρεται εώς τις 15:30",
            "category_info" : "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες"
        },
        {
            "name": "Ομελέτα χωριάτικη",
            "type": "Brunch",
            "category": "Ομελέτες",
            "price": "6.50",
            "discription" : "ελιές, φέτα, πράσινη πιπεριά, λουκάνικο, ρίγανη, ψωμί ολικής, ρόα, ντοματίνια, βαλσάμικο",
            "type_info" : "σερβίρεται εώς τις 15:30",
            "category_info" : "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες"
        },
        {
            "name": "Πρωινό 1",
            "type": "Brunch",
            "category": "Πρωινό",
            "price": "9.00",
            "discription" : "Ομελέτα επιλογής (αλλαντικών/τυριών), τορτίγια, με γαλοπούλα, τυρι, ντομάτα, μαγιονέζα, λόλα, σαλάτα, γραβίερα, pancakes",
            "type_info" : "σερβίρεται εώς τις 15:30"
        },
        {
            "name": "Πρωινό 2",
            "type": "Brunch",
            "category": "Πρωινό",
            "price": "9.00",
            "discription" : "Τοστάκια με τυρί, γαλοπούλα, φρυγανισμένο ψωμί, πατάτες τηγανιτές, ρολάκια μπέικον-λουκάνικο, ρολάκια τυρί-γαλοπούλα, pancakes",
            "type_info" : "σερβίρεται εώς τις 15:30"
        },
        {
            "name": "Bakan Breakfast",
            "type": "Brunch",
            "category": "Πρωινό",
            "price": "11.00",
            "discription" : "λουκάνικα, τηγανιτά αυγά, μπέικον, πατάτες τηγανιτές, σαλάτα, γραβιέρα, πέντε μίνι pancakes",
            "type_info" : "σερβίρεται εώς τις 15:30"
        },
        {
            "name": "Go Healthy",
            "type": "Brunch",
            "category": "Πρωινό",
            "price": "9.00",
            "discription" : "φέτες ψωμί ολικής με φυστικοβούτυρο, μέλι και μπανάνα, τορτίγια με γαλοπούλα, τυρί, φιλαδέλφεια, ντομάτα, λόλα σαλάτα",
            "type_info" : "σερβίρεται εώς τις 15:30"
        },
        {
            "name": "Power Bowl",
            "type": "Brunch",
            "category": "Πρωινό",
            "price": "6.50",
            "discription" : "τυρί κρέμα, γιαούρτι, μέλι, φρούτα εποχής, νιφάδες βρώμης, superfood mix",
            "type_info" : "σερβίρεται εώς τις 15:30"
        },
        {
            "name": "Αλλαντικών",
            "type": "Egg Tower",
            "category": "",
            "price": "5.50",
            "type_info" : "σερβίρεται εώς τις 15:00"
        },
        {
            "name": "Vegeterian",
            "type": "Egg Tower",
            "category": "",
            "price": "5.50",
            "type_info" : "σερβίρεται εώς τις 15:00"
        },
        {
            "name": "English",
            "type": "Egg Tower",
            "category": "",
            "price": "6.00",
            "type_info" : "σερβίρεται εώς τις 15:00"
        },
        {
            "name": "Classic",
            "type": "All Day",
            "category": "Club Sandwiches",
            "price": "5.50"
        },
        {
            "name": "Chicken",
            "type": "All Day",
            "category": "Club Sandwiches",
            "price": "6.00"
        },
        {
            "name": "Barbeque",
            "type": "All Day",
            "category": "Burgers",
            "price": "8.50"
        },
        {
            "name": "Chicken",
            "type": "All Day",
            "category": "Burgers",
            "price": "7.50"
        },
        {
            "name": "Special",
            "type": "All Day",
            "category": "Burgers",
            "price": "9.00"
        },
        {
            "name": "Σοκολάτα & Μπισκότο",
            "type": "Waffles",
            "category": "Γλυκές",
            "price": "5.00"
        },
        {
            "name": "Bueno & Μπισκότο Oreo",
            "type": "Waffles",
            "category": "Γλυκές",
            "price": "5.50"
        },
        {
            "name": "Lila Pause",
            "type": "Waffles",
            "category": "Γλυκές",
            "price": "5.50"
        },
        {
            "name": "Φρούτα & Μέλι",
            "type": "Waffles",
            "category": "Γλυκές",
            "discription": "μπανανα, μήλο, σταφίδες, μέλι, κανέλα",
            "price": "6.00"
        },
        {
            "name": "Ανάμεικτα",
            "type": "Waffles",
            "category": "Γλυκές",
            "discription": "πραλίνα σοκολάτα & bueno",
            "price": "5.50"
        },
        {
            "name": "Φυστικοβούτηρο",
            "type": "Waffles",
            "category": "Γλυκές",
            "discription": "μαρμελάδα φράουλα",
            "price": "6.00"
        },
        {
            "name": "Φυστικοβούτηρο",
            "type": "Waffles",
            "category": "Γλυκές",
            "discription": "σοκολάτα",
            "price": "6.00"
        },
        {
            "name": "Classic",
            "type": "Waffles",
            "category": "Αλμυρές",
            "discription": "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα",
            "price": "5.50"
        },
        {
            "name": "Special",
            "type": "Waffles",
            "category": "Αλμυρές",
            "discription": "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα, τηγανιτό αυγό",
            "price": "6.00"
        },
        {
            "name": "Σοκολάτα & Μπισκότο",
            "type": "Pancakes",
            "category": "Γλυκά",
            "price": "5.00",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Bueno & Μπισκότο Oreo",
            "type": "Pancakes",
            "category": "Γλυκά",
            "price": "5.50",
            "type_info": "Μέγεθος: 8 Μίνι / 3 Κανονικά.\nΠαγωτο +1.50,\nΜπανανα +1.00"
        },
        {
            "name": "Lila Pause",
            "type": "Pancakes",
            "category": "Γλυκά",
            "price": "5.50",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Φρούτα & Μέλι",
            "type": "Pancakes",
            "category": "Γλυκά",
            "discription": "μπανανα, μήλο, σταφίδες, μέλι, κανέλα",
            "price": "6.00",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Ανάμεικτα",
            "type": "Pancakes",
            "category": "Γλυκά",
            "discription": "πραλίνα σοκολάτα & bueno",
            "price": "5.50",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Φυστικοβούτηρο",
            "type": "Pancakes",
            "category": "Γλυκά",
            "discription": "μαρμελάδα φράουλα",
            "price": "6.00",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Φυστικοβούτηρο",
            "type": "Pancakes",
            "category": "Γλυκά",
            "discription": "σοκολάτα",
            "price": "6.00",
            "type_info" : "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Classic",
            "type": "Pancakes",
            "category": "Αλμυρά",
            "discription": "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα",
            "price": "5.50",
            "type_info" : "Μέγεθος: 8 Μίνι/3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Special",
            "type": "Pancakes",
            "category": "Αλμυρά",
            "discription": "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα, τηγανιτό αυγό",
            "price": "6.00",
            "type_info" : "Μέγεθος: 8 Μίνι/3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00"
        },
        {
            "name": "Chicken Nuggets",
            "type": "Single Plates",
            "category": "",
            "discription": "παναρισμένες μπουκίτσες κοτόπουλο, πατάτες τηγανιτές, French sauce",
            "price": "6.50"
        },
        {
            "name": "Cheddar Fries",
            "type": "Single Plates",
            "category": "",
            "discription": "πατάτες τηγανιτές με cheddar sause & μπέικον",
            "price": "3.80"
        },
        {
            "name": "Hot Dog",
            "type": "Single Plates",
            "category": "",
            "discription": "μαλακό ψωμάκι, λουκάνικο Φρανγκφούρτης, κέτσαπ, μουστάρδα, coleslaw, πατάτες τηγανιτές",
            "price": "6.00"
        },
        {
            "name": "Open Special Bread",
            "type": "Single Plates",
            "category": "",
            "discription": "μαλακό ψωμάκι, πατάτες τηγανιτές, τυρί, γαλοπούλα ωμοπλάτη, λουκάνικο, μπέικον, french sause & μαγιονέζα",
            "price": "7.00"
        },
        {
            "name": "Ceasar's Salad",
            "type": "Single Plates",
            "category": "",
            "discription": "μαρούλι, κόκκινη λόλα, φιλετακια κοτόπουλο, ντοματίνια, κρουτόν, γραβιέρα, ceasar's sauce",
            "price": "6.00"
        },
        {
            "name": "BAKAN Salad",
            "type": "Single Plates",
            "category": "",
            "discription": "λόλα, ρόκα, ντοματίνια, super mix nuts, μήλο, γραβιέρα, αγγούρι, κρουτόν, βαλσάμικο",
            "price": "6.50"
        },
        {
            "name": "Ποικιλία Τυριών/Αλλαντικών",
            "type": "Single Plates",
            "category": "",
            "discription": "μπέικον, λουκάνικο Φρανγκφούρτης, ωμοπλάτη, γαλοπούλα, γραβιέρα, τυρί ένταμ, κριτσίνια",
            "price": "6.50"
        }
    
    
    ]
}