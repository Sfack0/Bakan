const menuDataGR = {
  drinks: [
    {
      name: "Espresso",
      type: "Καφές",
      category: "Ζεστοί",
      price: "2.00/2.40/2.90",
    },
    {
      name: "Espresso Macchiato",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.00",
    },
    {
      name: "Americano",
      type: "Καφές",
      category: "Ζεστοί",
      price: "2.50/3.00",
    },
    {
      name: "Cappuccino",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.20/3.80/4.20",
    },
    {
      name: "Flat White",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.50",
    },
    {
      name: "Latte",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.50",
    },
    {
      name: "Latte Caramel",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.80",
    },
    {
      name: "Φίλτρου",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.50",
    },
    {
      name: "Mocha",
      type: "Καφές",
      category: "Ζεστοί",
      price: "4.00",
    },
    {
      name: "Ελληνικός",
      type: "Καφές",
      category: "Ζεστοί",
      price: "2.00/2.50",
    },
    {
      name: "Nescafe",
      type: "Καφές",
      category: "Ζεστοί",
      price: "3.00",
    },
    {
      name: "Freddo Espresso",
      type: "Καφές",
      category: "Κρύοι",
      price: "3.50/4.00",
    },
    {
      name: "Freddo Cappuccino",
      type: "Καφές",
      category: "Κρύοι",
      price: "3.50/4.00",
    },
    {
      name: "Iced Latte",
      type: "Καφές",
      category: "Κρύοι",
      price: "3.50",
    },
    {
      name: "Iced Latte Caramel",
      type: "Καφές",
      category: "Κρύοι",
      price: "3.80",
    },
    {
      name: "Freddo Caramel",
      type: "Καφές",
      category: "Κρύοι",
      price: "4.00",
    },
    {
      name: "Iced Mochaccino",
      type: "Καφές",
      category: "Κρύοι",
      price: "4.00",
    },
    {
      name: "Frappe",
      type: "Καφές",
      category: "Κρύοι",
      price: "3.00",
    },
    {
      name: "Feeju ice tea ροδάκινο",
      type: "Τσάι",
      category: "Παγωμένο",
      price: "3.50",
    },
    {
      name: "Feeju ice tea λεμόνι",
      type: "Τσάι",
      category: "Παγωμένο",
      price: "3.50",
    },
    {
      name: "English Breakfast",
      type: "Τσάι",
      category: "Μαύρο",
      price: "3.50",
      img: "../images/tea.png",
    },
    {
      name: "Early Grey Blue Flower",
      type: "Τσάι",
      category: "Μαύρο",
      price: "3.50",
    },
    {
      name: "Chai Tea",
      type: "Τσάι",
      category: "Μαύρο",
      price: "3.50",
    },
    {
      name: "Darjeeling Margaret's Hope",
      type: "Τσάι",
      category: "Μαύρο",
      price: "3.50",
    },

    {
      name: "Chia Gunpowder",
      type: "Τσάι",
      category: "Πράσινο",
      price: "3.50",
    },
    {
      name: "China Jasmine",
      type: "Τσάι",
      category: "Πράσινο",
      price: "3.50",
    },
    {
      name: "Fruit Melange 'Christ'",
      type: "Τσάι",
      category: "Κόκκινο",
      price: "3.50",
    },
    {
      name: "Rooitea 'Capetown'",
      type: "Τσάι",
      category: "Κόκκινο",
      price: "3.50",
    },
    {
      name: "Cream Caramel",
      type: "Τσάι",
      category: "Κόκκινο",
      price: "3.50",
    },
    {
      name: "Fireplace",
      type: "Τσάι",
      category: "Κόκκινο",
      price: "3.50",
    },
    {
      name: "Apple Ginger",
      type: "Τσάι",
      category: "Ανάμεικτο",
      price: "3.50",
    },
    {
      name: "Καρτεράκι",
      type: "Τσάι",
      category: "Ανάμεικτο",
      price: "3.50",
    },

    {
      name: "Κλασικη ζεστή/κρύα",
      type: "Σοκολάτες",
      category: "",
      price: "4.00",
      type_info: "σιρόπι +0.40 , σαντιγί +0.40",
    },
    {
      name: "Λευκή Σοκολάτα",
      type: "Σοκολάτες",
      category: "",
      price: "4.00",
      type_info: "σιρόπι +0.40 , σαντιγί +0.40",
    },
    {
      name: "Γρανίτα σοκολάτας",
      type: "Σοκολάτες",
      category: "",
      price: "4.00",
      type_info: "σιρόπι +0.40 , σαντιγί +0.40",
    },
    {
      name: "Γεύσεις",
      type: "Σοκολάτες",
      category: "",
      description:
        "Πραλίνα Φουντουκιού / Φράουλα / Καραμέλα με θαλασσινό αλάτι / Bitter, Φυστικοβούτυρο-Μπανάνα, Lemon pie",
      price: "4.20",
      type_info: "σιρόπι +0.40 , σαντιγί +0.40",
    },
    {
      name: "Vanilla Caramel",
      type: "Special Ροφήματα",
      category: "Ζεστά",
      price: "4.50",
    },
    {
      name: "Vanilla Biscuit",
      type: "Special Ροφήματα",
      category: "Ζεστά",
      price: "4.50",
    },
    {
      name: "Mocha hazelnut",
      type: "Special Ροφήματα",
      category: "Ζεστά",
      price: "4.50",
    },
    {
      name: "Vanilla Ice",
      type: "Special Ροφήματα",
      category: "Κρύα",
      price: "4.20",
    },
    {
      name: "Vanilla Ice Caramel",
      type: "Special Ροφήματα",
      category: "Κρύα",
      price: "4.50",
    },
    {
      name: "Full Chocolate Ice",
      type: "Special Ροφήματα",
      category: "Κρύα",
      price: "4.50",
    },
    {
      name: "Milk Shake",
      type: "Special Ροφήματα",
      category: "Κρύα",
      price: "5.50",
    },
    {
      name: "Μπύρα Άλφα 330ml",
      type: "Αλκοόλ",
      category: "",
      price: "3.50",
    },
    {
      name: "Μπύρα Fix 330ml",
      type: "Αλκοόλ",
      category: "",
      price: "3.50",
    },
    {
      name: "Μπύρα Kaizer 330ml",
      type: "Αλκοόλ",
      category: "",
      price: "4.00",
    },
    {
      name: "Βότκα ποτήρι",
      type: "Αλκοόλ",
      category: "",
      price: "6.00",
    },
    {
      name: "Ουίσκι ποτήρι",
      type: "Αλκοόλ",
      category: "",
      price: "6.00",
    },
    {
      name: "Παγολεμονάδα",
      type: "Χυμοί",
      category: "",
      price: "3.50",
    },
    {
      name: "Παγοπορτοκαλάδα",
      type: "Χυμοί",
      category: "",
      price: "3.50",
    },
    {
      name: "Κανελάδα ζεστή/κρύα",
      type: "Χυμοί",
      category: "",
      price: "3.50",
    },
    {
      name: "Σπιτικές λεμονάδες B Fresh",
      type: "Χυμοί",
      description: "Μαστίχα Χίου/Ginger",
      category: "",
      price: "4.00",
    },
    {
      name: "Φυσικός χυμός πορτοκάλι",
      type: "Χυμοί",
      category: "",
      price: "4.00",
    },
    {
      name: "Φυσικός χυμός ανάμεικτος",
      type: "Χυμοί",
      category: "",
      price: "4.50",
    },
    {
      name: "Φυσικός χυμός μπανάνας",
      type: "Χυμοί",
      description: "γάλα αμυγδάλου +0.50",
      category: "",
      price: "4.00",
    },
    {
      name: "Coca Cola / zero",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Fanta blue/lemon/orange",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Sprite",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Schweppes soda/grapefruit",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita motion",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita supermix",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita cherry",
      type: "Αναψυκτικά",
      category: "",
      price: "3.20",
    },

    {
      name: "Monster πράσινο/άσπρο/μπλε",
      type: "Αναψυκτικά",
      category: "",
      price: "3.50",
    },
    {
      name: "Ανθρακούχο νερό ζαρός",
      type: "Αναψυκτικά",
      category: "",
      price: "2.80",
    },
    {
      name: "Ανθρακούχο νερό ζαρός με λεμόνι",
      type: "Αναψυκτικά",
      category: "",
      price: "2.80",
    },
    {
      name: "Νερό ζαρός 0.5lt",
      type: "Αναψυκτικά",
      category: "",
      price: "0.50",
    },
    {
      name: "Νερό ζαρός 1lt",
      type: "Αναψυκτικά",
      category: "",
      price: "1.00",
    },
  ],
  food: [
    {
      name: "Ομελέτα αλλαντικών",
      type: "Brunch",
      category: "Ομελέτες",
      price: "7.00",
      description:
        "μπέικον, γαλοπούλα, λουκάνικο Φρανγκφουρτης, ψωμί ολικής, μαγιονέζα, ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:30",
      category_info: "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες",
    },
    {
      name: "Ομελέτα τυριών",
      type: "Brunch",
      category: "Ομελέτες",
      price: "7.00",
      description:
        "γραβιέρα, gouda, edam, μανούρι, κρέμα αβοκάντο, ψωμί ολικής, ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:30",
      category_info: "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες",
    },
    {
      name: "Ομελέτα χωριάτικη",
      type: "Brunch",
      category: "Ομελέτες",
      price: "8.00",
      description:
        "ελιές, μανούρι, πράσινη πιπεριά, λουκάνικο, ρίγανη, ψωμί ολικής, ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:30",
      category_info: "όλες οι ομελέτες σερβίρονται με τηγανιτές πατάτες",
    },
    {
      name: "Πρωινό 1",
      type: "Brunch",
      category: "Πρωινό",
      price: "11.00",
      description:
        "Ομελέτα επιλογής (αλλαντικών/τυριών), τορτίγια, με γαλοπούλα, τυρι, ντομάτα, φιλαδέλφια, λόλα, σαλάτα, γραβίερα, pancakes",
      type_info: "σερβίρεται εώς τις 15:30",
    },
    {
      name: "Πρωινό 2",
      type: "Brunch",
      category: "Πρωινό",
      price: "11.00",
      description:
        "Τοστάκια με τυρί, γαλοπούλα, φρυγανισμένο ψωμί, πατάτες τηγανιτές, ρολάκια μπέικον-λουκάνικο, ρολάκια τυρί-γαλοπούλα, pancakes",
      type_info: "σερβίρεται εώς τις 15:30",
    },
    {
      name: "Πρωινό Bakan",
      type: "Brunch",
      category: "Πρωινό",
      price: "13.00",
      description:
        "λουκάνικα, τηγανιτά αυγά, μπέικον, πατάτες τηγανιτές, σαλάτα, γραβιέρα, πέντε pancakes",
      type_info: "σερβίρεται εώς τις 15:30",
    },
    {
      name: "Go Healthy",
      type: "Brunch",
      category: "Πρωινό",
      price: "10.00",
      description:
        "φέτες ψωμί ολικής με φυστικοβούτυρο, μέλι και μπανάνα, τορτίγια με γαλοπούλα, τυρί, φιλαδέλφεια, ντομάτα, λόλα σαλάτα",
      type_info: "σερβίρεται εώς τις 15:30",
    },
    {
      name: "Power Bowl",
      type: "Brunch",
      category: "Πρωινό",
      price: "8.00",
      description:
        "τυρί κρέμα, γιαούρτι, μέλι, φρούτα εποχής, νιφάδες βρώμης, superfood mix",
      type_info: "σερβίρεται εώς τις 15:30",
    },
    {
      name: "Αλλαντικών",
      type: "Egg Tower",
      category: "",
      price: "6.00",
      description:
        "φέτες ψωμί ολικής, τυρί, γαλοπούλα, μπέικον, κρέμα, αβοκάντο, τηγανιτά αυγά, αλάτι, πιπέρι, σαλάτα με ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:00",
      category_info: "Όλα τα αυγά μας είναι φρέσκα ελευθέρας βοσκής",
    },
    {
      name: "Vegeterian",
      type: "Egg Tower",
      category: "",
      price: "6.00",
      description:
        "φέτες ψωμί ολικής, τυρί κρέμα αβοκάντο, τηγανιτά αυγά, αλάτι, πιπέρι, σαλάτα με ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:00",
      category_info: "Όλα τα αυγά μας είναι φρέσκα ελευθέρας βοσκής",
    },
    {
      name: "English",
      type: "Egg Tower",
      category: "",
      price: "6.00",
      description:
        "φέτες ψωμί ολικής, κρέμα αβοκάντο cheddar, μπέικον, τηγανιτά αυγά, αλάτι, πιπέρι, σαλάτα με ρόκα, ντοματίνια, βαλσάμικο",
      type_info: "σερβίρεται εώς τις 15:00",
      category_info: "Όλα τα αυγά μας είναι φρέσκα ελευθέρας βοσκής",
    },
    {
      name: "Classic",
      type: "All Day",
      category: "Club Sandwiches",
      price: "6.00",
      description:
        "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, λόλα, ντομάτα, πατάτες τηγανιτές",
    },
    {
      name: "Chicken",
      type: "All Day",
      category: "Club Sandwiches",
      price: "7.00",
      description:
        "τυρί, φιλετάκια κοτόπουλο, μπέικον, mixed chicken sauce, λόλα, ντομάτα, πατάτες τηγανιτές",
    },
    {
      name: "Barbeque",
      type: "All Day",
      category: "Burgers",
      price: "9.00",
      description:
        "ψωμάκι brioche 100% μοσχαρίσιο μπιφτέκι 200γρ, μπέικον, golden bbq sauce, cheddar, καραμελωμένα κρεμμύδια, λόλα, ντομάτα, πατάτες τηγανιτές",
    },
    {
      name: "Chicken",
      type: "All Day",
      category: "Burgers",
      price: "8.00",
      description:
        "ψωμάκι brioche, παναρισμένο φιλέτο κοτόπουλο, μαγιονέζα, cheddar, λόλα, ντομάτα, πατάτες τηγανιτές",
    },
    {
      name: "Special",
      type: "All Day",
      category: "Burgers",
      price: "10.00",
      description:
        "ψωμάκι brioche, 100% μοσχαρίσιο μπιφτέκι 200γρ, ομελέτα, μπέικον, golden bbq sauce, cheddar, καραμελωμένα κρεμμύδια, λόλα, ντομάτα, πατάτες τηγανιτές",
    },
    {
      name: "Σοκολάτα & Μπισκότο",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "6.00",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Bueno & Μπισκότο Oreo",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "6.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "6.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & mix nuts",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "7.00",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & μπανάνα",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "7.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & mix nuts & μπανάνα",
      type: "Βάφλες",
      category: "Γλυκές",
      price: "7.90",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Φρούτα & Μέλι",
      type: "Βάφλες",
      category: "Γλυκές",
      description: "μπανανα, μήλο, mix nuts, μέλι, κανέλα",
      price: "7.00",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Ανάμεικτο",
      type: "Βάφλες",
      category: "Γλυκές",
      description: "πραλίνα σοκολάτα & bueno",
      price: "6.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Classic",
      type: "Βάφλες",
      category: "Αλμυρές",
      description: "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα",
      price: "6.50",
    },
    {
      name: "Special",
      type: "Βάφλες",
      category: "Αλμυρές",
      description: "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα, τηγανιτό αυγό",
      price: "7.00",
    },
    {
      name: "Σοκολάτα & Μπισκότο",
      type: "Pancakes",
      category: "Γλυκά",
      price: "6.00",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά.\n Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Bueno & Μπισκότο Oreo",
      type: "Pancakes",
      category: "Γλυκά",
      price: "6.50",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά.Παγωτο +1.50,Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης",
      type: "Pancakes",
      category: "Γλυκά",
      price: "6.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & mix nuts",
      type: "Pancakes",
      category: "Γλυκά",
      price: "7.00",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & μπανάνα",
      type: "Pancakes",
      category: "Γλυκά",
      price: "7.50",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα φυστίκι Αιγίνης & mix nuts & μπανάνα",
      type: "Pancakes",
      category: "Γλυκά",
      price: "7.90",
      type_info: "Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Φρούτα & Μέλι",
      type: "Pancakes",
      category: "Γλυκά",
      description: "μπανανα, μήλο, mix nuts, μέλι, κανέλα",
      price: "7.00",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Πραλίνα σοκολάτα & bueno",
      type: "Pancakes",
      category: "Γλυκά",
      price: "6.50",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Φυστικοβούτηρο & σοκολάτα",
      type: "Pancakes",
      category: "Γλυκά",
      price: "6.50",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Classic",
      type: "Pancakes",
      category: "Αλμυρά",
      description: "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα",
      price: "6.50",
      type_info: "Μέγεθος: 8 Μίνι / 3 Κανονικά. Παγωτο +1.50, Μπανανα +1.00",
    },
    {
      name: "Special",
      type: "Pancakes",
      category: "Αλμυρά",
      description: "τυρί, γαλοπούλα, μπέικον, μαγιονέζα, ρόκα, τηγανιτό αυγό",
      price: "7.00",
    },
    {
      name: "Παγωτό Βανίλια",
      type: "Παγωτά",
      type_info: "1 μπάλα / 2 μπάλες / 3 μπάλες",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Παγωτό Μπανάνα",
      type: "Παγωτά",
      type_info: "1 μπάλα / 2 μπάλες / 3 μπάλες",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Παγωτό Φράουλα",
      type: "Παγωτά",
      type_info: "1 μπάλα / 2 μπάλες / 3 μπάλες",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Παγωτό Φυστίκι",
      type: "Παγωτά",
      type_info: "1 μπάλα / 2 μπάλες / 3 μπάλες",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Chicken Nuggets",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "παναρισμένες μπουκίτσες κοτόπουλο, πατάτες τηγανιτές, French sauce",
      price: "7.50",
    },
    {
      name: "Τηγανιτές Πατάτες",
      type: "Ατομικές Μερίδες",
      category: " ",
      description: "πατάτες τηγανιτές με cheddar sause & μπέικον",
      price: "4.20",
    },
    {
      name: "Hot Dog",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "μαλακό ψωμάκι, λουκάνικο Φρανγκφούρτης, κέτσαπ, μουστάρδα, coleslaw, πατάτες τηγανιτές",
      price: "6.00",
    },
    {
      name: "Open Special Bread",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "μαλακό ψωμάκι, πατάτες τηγανιτές, τυρί, γαλοπούλα ωμοπλάτη, λουκάνικο, μπέικον, french sause & μαγιονέζα",
      price: "8.00",
    },
    {
      name: "Ceasar's Salad",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "λόλα, φιλετακια κοτόπουλο, ντοματίνια, κρουτόν, γραβιέρα, ceasar's sauce",
      price: "7.50",
    },
    {
      name: "BAKAN Salad",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "λόλα, ρόκα, ντοματίνια, super mix nuts, μήλο, γραβιέρα, αγγούρι, κρουτόν, βαλσάμικο",
      price: "8.00",
    },
    {
      name: "Ποικιλία Τυριών/Αλλαντικών",
      type: "Ατομικές Μερίδες",
      category: " ",
      description:
        "μπέικον, λουκάνικο Φρανγκφούρτης, ωμοπλάτη, γαλοπούλα, γραβιέρα, τυρί ένταμ, κριτσίνια",
      price: "8.00",
    },
  ],
};

const menuDataENG = {
  drinks: [
    {
      name: "Espresso",
      type: "Coffee",
      category: "Hot",
      price: "2.00/2.40/2.90",
    },
    {
      name: "Espresso Macchiato",
      type: "Coffee",
      category: "Hot",
      price: "3.00",
    },
    {
      name: "Americano",
      type: "Coffee",
      category: "Hot",
      price: "2.50/3.00",
    },
    {
      name: "Cappuccino",
      type: "Coffee",
      category: "Hot",
      price: "3.20/3.80/4.20",
    },
    {
      name: "Flat White",
      type: "Coffee",
      category: "Hot",
      price: "3.50",
    },
    {
      name: "Latte",
      type: "Coffee",
      category: "Hot",
      price: "3.50",
    },
    {
      name: "Caramel Latte",
      type: "Coffee",
      category: "Hot",
      price: "3.80",
    },
    {
      name: "Filter Coffee",
      type: "Coffee",
      category: "Hot",
      price: "3.50",
    },
    {
      name: "Mocha",
      type: "Coffee",
      category: "Hot",
      price: "4.00",
    },
    {
      name: "Greek Coffee",
      type: "Coffee",
      category: "Hot",
      price: "2.00/2.50",
    },
    {
      name: "Nescafé",
      type: "Coffee",
      category: "Hot",
      price: "3.00",
    },
    {
      name: "Freddo Espresso",
      type: "Coffee",
      category: "Cold",
      price: "3.50/4.00",
    },
    {
      name: "Freddo Cappuccino",
      type: "Coffee",
      category: "Cold",
      price: "3.50/4.00",
    },
    {
      name: "Iced Latte",
      type: "Coffee",
      category: "Cold",
      price: "3.50",
    },
    {
      name: "Iced Caramel Latte",
      type: "Coffee",
      category: "Cold",
      price: "3.80",
    },
    {
      name: "Freddo Caramel",
      type: "Coffee",
      category: "Cold",
      price: "4.00",
    },
    {
      name: "Iced Mochaccino",
      type: "Coffee",
      category: "Cold",
      price: "4.00",
    },
    {
      name: "Frappe",
      type: "Coffee",
      category: "Cold",
      price: "3.00",
    },
    {
      name: "Feeju ice tea Peach",
      type: "Tea",
      category: "Cold",
      price: "3.50",
    },
    {
      name: "Feeju ice tea Lemon",
      type: "Tea",
      category: "Cold",
      price: "3.50",
    },
    {
      name: "English Breakfast",
      type: "Tea",
      category: "Black",
      price: "3.50",
      img: "../images/tea.png",
    },
    {
      name: "Earl Grey Blue Flower",
      type: "Tea",
      category: "Black",
      price: "3.50",
    },
    {
      name: "Chai Tea",
      type: "Tea",
      category: "Black",
      price: "3.50",
    },
    {
      name: "Darjeeling Margaret's Hope",
      type: "Tea",
      category: "Black",
      price: "3.50",
    },
    {
      name: "China Gunpowder",
      type: "Tea",
      category: "Green",
      price: "3.50",
    },

    {
      name: "China Jasmine",
      type: "Tea",
      category: "Green",
      price: "3.50",
    },
    {
      name: "Fruit Melange 'Christ'",
      type: "Tea",
      category: "Red",
      price: "3.50",
    },
    {
      name: "Rooibos 'Capetown'",
      type: "Tea",
      category: "Red",
      price: "3.50",
    },
    {
      name: "Cream Caramel",
      type: "Tea",
      category: "Red",
      price: "3.50",
    },
    {
      name: "Fireplace",
      type: "Tea",
      category: "Red",
      price: "3.50",
    },
    {
      name: "Apple Ginger",
      type: "Tea",
      category: "Mixed",
      price: "3.50",
    },
    {
      name: "Karteraki",
      type: "Tea",
      category: "Mixed",
      price: "3.50",
    },
    {
      name: "Classic Hot/Cold",
      type: "Chocolate",
      category: "",
      price: "4.00",
      type_info: "syrup +0.40, whipped cream +0.40",
    },
    {
      name: "White Chocolate",
      type: "Chocolate",
      category: "",
      price: "4.00",
      type_info: "syrup +0.40, whipped cream +0.40",
    },
    {
      name: "Chocolate Granita",
      type: "Chocolate",
      category: "",
      price: "4.00",
      type_info: "syrup +0.40, whipped cream +0.40",
    },
    {
      name: "Flavors",
      type: "Chocolate",
      category: "",
      description:
        "0% Sugar / Hazelnut Praline / Strawberry / Salted Caramel / Bueno / Bitter, Peanut butter-banana, Lemon pie",
      price: "4.20",
      type_info: "syrup +0.40, whipped cream +0.40",
    },
    {
      name: "Vanilla Caramel",
      type: "Special Drinks",
      category: "Hot",
      price: "4.50",
    },
    {
      name: "Vanilla Biscuit",
      type: "Special Drinks",
      category: "Hot",
      price: "4.50",
    },
    {
      name: "Mocha Hazelnut",
      type: "Special Drinks",
      category: "Hot",
      price: "4.50",
    },
    {
      name: "Vanilla Ice",
      type: "Special Drinks",
      category: "Cold",
      price: "4.20",
    },
    {
      name: "Vanilla Ice Caramel",
      type: "Special Drinks",
      category: "Cold",
      price: "4.50",
    },
    {
      name: "Full Chocolate Ice",
      type: "Special Drinks",
      category: "Cold",
      price: "4.50",
    },
    {
      name: "Milk Shake",
      type: "Special Drinks",
      category: "Cold",
      price: "5.50",
    },
    {
      name: "Alpha Beer 330ml",
      type: "Alcohol",
      category: "",
      price: "3.50",
    },
    {
      name: "Fix Beer 330ml",
      type: "Alcohol",
      category: "",
      price: "3.50",
    },
    {
      name: "Kaizer Beer 330ml",
      type: "Alcohol",
      category: "",
      price: "4.00",
    },
    {
      name: "Vodka (glass)",
      type: "Alcohol",
      category: "",
      price: "6.00",
    },
    {
      name: "Whiskey (glass)",
      type: "Alcohol",
      category: "",
      price: "6.00",
    },

    {
      name: "Iced Lemonade",
      type: "Juices",
      category: "",
      price: "3.50",
    },
    {
      name: "Iced Orangeade",
      type: "Juices",
      category: "",
      price: "3.50",
    },
    {
      name: "Cinnamon Drink Hot/Cold",
      type: "Juices",
      category: "",
      price: "3.50",
    },
    {
      name: "Homemade Lemonades B Fresh",
      type: "Juices",
      description: "Chios Mastiha / Ginger / Grapefruit / Sour Cherry",
      category: "",
      price: "4.00",
    },
    {
      name: "Fresh Orange Juice",
      type: "Juices",
      category: "",
      price: "4.00",
    },
    {
      name: "Fresh Mixed Juice",
      type: "Juices",
      category: "",
      price: "4.50",
    },
    {
      name: "Fresh Banana Juice",
      type: "Juices",
      description: "almond milk +0.50",
      category: "",
      price: "4.00",
    },
    {
      name: "Coca Cola / Zero",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Fanta Blue/Lemon/Orange",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Sprite",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Schweppes Soda/Grapefruit",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita Motion",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita Supermix",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },
    {
      name: "Amita Cherry",
      type: "Soft Drinks",
      category: "",
      price: "3.20",
    },

    {
      name: "Monster Green/White/Blue",
      type: "Soft Drinks",
      category: "",
      price: "3.50",
    },
    {
      name: "Zaros Sparkling Water",
      type: "Soft Drinks",
      category: "",
      price: "2.80",
    },
    {
      name: "Zaros Sparkling Water with Lemon",
      type: "Soft Drinks",
      category: "",
      price: "2.80",
    },
    {
      name: "Zaros Water 0.5L",
      type: "Soft Drinks",
      category: "",
      price: "0.50",
    },
    {
      name: "Zaros Water 1L",
      type: "Soft Drinks",
      category: "",
      price: "1.00",
    },
  ],
  food: [
    {
      name: "Cold Cuts Omelette",
      type: "Brunch",
      category: "Omelettes",
      price: "6.00",
      description:
        "bacon, turkey, Frankfurt sausage, whole wheat bread, mayonnaise, arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:30",
      category_info: "all omelettes are served with fries",
    },
    {
      name: "Cheese Omelette",
      type: "Brunch",
      category: "Omelettes",
      price: "6.00",
      description:
        "graviera, gouda, edam, manouri cheese, avocado cream, whole wheat bread, arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:30",
      category_info: "all omelettes are served with fries",
    },
    {
      name: "Village Omelette",
      type: "Brunch",
      category: "Omelettes",
      price: "6.50",
      description:
        "olives, manouri cheese, green pepper, sausage, oregano, whole wheat bread, arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:30",
      category_info: "all omelettes are served with fries",
    },
    {
      name: "Breakfast 1",
      type: "Brunch",
      category: "Breakfast",
      price: "9.00",
      description:
        "Omelette of choice (cold cuts/cheese), tortilla with turkey, cheese, tomato, philadelphia, lollo lettuce, salad, graviera, pancakes",
      type_info: "served until 15:30",
    },
    {
      name: "Breakfast 2",
      type: "Brunch",
      category: "Breakfast",
      price: "9.00",
      description:
        "Toast with cheese, turkey, toasted bread, fries, bacon-sausage rolls, cheese-turkey rolls, pancakes",
      type_info: "served until 15:30",
    },
    {
      name: "Bakan Breakfast",
      type: "Brunch",
      category: "Breakfast",
      price: "11.00",
      description:
        "sausages, fried eggs, bacon, fries, salad, graviera, five pancakes",
      type_info: "served until 15:30",
    },
    {
      name: "Go Healthy",
      type: "Brunch",
      category: "Breakfast",
      price: "9.00",
      description:
        "whole wheat bread slices with peanut butter, honey, and banana, tortilla with turkey, cheese, Philadelphia, tomato, lollo lettuce",
      type_info: "served until 15:30",
    },
    {
      name: "Power Bowl",
      type: "Brunch",
      category: "Breakfast",
      price: "6.50",
      description:
        "cream cheese, yogurt, honey, seasonal fruits, oat flakes, superfood mix",
      type_info: "served until 15:30",
    },
    {
      name: "Cold Cuts",
      type: "Egg Tower",
      category: "",
      price: "5.50",
      description:
        "whole wheat bread slices, cheese, turkey, bacon, avocado cream, fried eggs, salt, pepper, salad with arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:00",
      category_info: "All our eggs are fresh and free-range",
    },
    {
      name: "Vegeterian",
      type: "Egg Tower",
      category: "",
      price: "5.50",
      description:
        "whole wheat bread slices, avocado cream cheese, fried eggs, salt, pepper, salad with arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:00",
      category_info: "All our eggs are fresh and free-range",
    },
    {
      name: "English",
      type: "Egg Tower",
      category: "",
      price: "6.00",
      description:
        "whole wheat bread slices, avocado cheddar cream, bacon, fried eggs, salt, pepper, salad with arugula, cherry tomatoes, balsamic",
      type_info: "served until 15:00",
      category_info: "All our eggs are fresh and free-range",
    },
    {
      name: "Classic",
      type: "All Day",
      category: "Club Sandwiches",
      price: "5.50",
      description:
        "cheese, turkey, bacon, mayonnaise, lollo lettuce, tomato, fries",
    },
    {
      name: "Chicken",
      type: "All Day",
      category: "Club Sandwiches",
      price: "6.00",
      description:
        "cheese, chicken fillets, bacon, mixed chicken sauce, lollo lettuce, tomato, fries",
    },
    {
      name: "Barbeque",
      type: "All Day",
      category: "Burgers",
      price: "8.50",
      description:
        "brioche bun, 100% beef patty 200g, bacon, golden BBQ sauce, cheddar, caramelized onions, lollo lettuce, tomato, fries",
    },
    {
      name: "Chicken",
      type: "All Day",
      category: "Burgers",
      price: "7.50",
      description:
        "brioche bun, breaded chicken fillet, mayonnaise, cheddar, lollo lettuce, tomato, fries",
    },
    {
      name: "Special",
      type: "All Day",
      category: "Burgers",
      price: "9.00",
      description:
        "brioche bun, 100% beef patty 200g, omelette, bacon, golden BBQ sauce, cheddar, caramelized onions, lollo lettuce, tomato, fries",
    },
    {
      name: "Chocolate & Biscuit",
      type: "Waffles",
      category: "Sweet",
      price: "5.00",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Bueno & Oreo Biscuit",
      type: "Waffles",
      category: "Sweet",
      price: "5.50",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina",
      type: "Waffles",
      category: "Sweet",
      price: "6.50",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & mix nuts",
      type: "Waffles",
      category: "Sweet",
      price: "7.00",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & banana",
      type: "Waffles",
      category: "Sweet",
      price: "7.50",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & mix nuts & banana",
      type: "Waffles",
      category: "Sweet",
      price: "7.90",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Fruits & Honey",
      type: "Waffles",
      category: "Sweet",
      description: "banana, apple, mix nuts, honey, cinnamon",
      price: "6.00",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Mixed",
      type: "Waffles",
      category: "Sweet",
      description: "chocolate praline & bueno",
      price: "5.50",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Peanut Butter",
      type: "Waffles",
      category: "Sweet",
      description: "chocolate",
      price: "6.00",
      type_info: "Ice cream +1.50, banana +1.00",
    },
    {
      name: "Classic",
      type: "Waffles",
      category: "Savory",
      description: "cheese, turkey, bacon, mayonnaise, arugula",
      price: "5.50",
    },
    {
      name: "Special",
      type: "Waffles",
      category: "Savory",
      description: "cheese, turkey, bacon, mayonnaise, arugula, fried egg",
      price: "6.00",
    },
    {
      name: "Chocolate & Biscuit",
      type: "Pancakes",
      category: "Sweet",
      price: "5.00",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Bueno & Oreo Biscuit",
      type: "Pancakes",
      category: "Sweet",
      price: "5.50",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina",
      type: "Pancakes",
      category: "Sweet",
      price: "6.50",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & mix nuts",
      type: "Pancakes",
      category: "Sweet",
      price: "7.00",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & banana",
      type: "Pancakes",
      category: "Sweet",
      price: "7.50",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Pistachio Praline Aegina & mix nuts & banana",
      type: "Pancakes",
      category: "Sweet",
      price: "7.90",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Fruits & Honey",
      type: "Pancakes",
      category: "Sweet",
      description: "banana, apple, mix nuts, honey, cinnamon",
      price: "6.00",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Mixed",
      type: "Pancakes",
      category: "Sweet",
      description: "chocolate praline & bueno",
      price: "5.50",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Peanut Butter",
      type: "Pancakes",
      category: "Sweet",
      description: "strawberry jam",
      price: "6.00",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Peanut Butter",
      type: "Pancakes",
      category: "Sweet",
      description: "chocolate",
      price: "6.00",
      type_info: "Size: 8 Mini / 3 Regular.\nIce cream +1.50, Banana +1.00",
    },
    {
      name: "Classic",
      type: "Pancakes",
      category: "Savory",
      description: "cheese, turkey, bacon, mayonnaise, rocket",
      price: "5.50",
    },
    {
      name: "Special",
      type: "Pancakes",
      category: "Savory",
      description: "cheese, turkey, bacon, mayonnaise, rocket, fried egg",
      price: "6.00",
    },
    ,
    {
      name: "Ice Cream Vanilla",
      type: "Ice Cream",
      category: " ",
      type_info: "1 scoop / 2 scoops / 3 scoops",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Ice Cream Banana",
      type: "Ice Cream",
      type_info: "1 scoop / 2 scoops / 3 scoops",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Ice Cream Strawberry",
      type: "Ice Cream",
      type_info: "1 scoop / 2 scoops / 3 scoops",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Ice Cream Peanut",
      type: "Ice Cream",
      type_info: "1 scoop / 2 scoops / 3 scoops",
      category: " ",
      description: "",
      price: "2.00/3.80/5.00",
    },
    {
      name: "Chicken Nuggets",
      type: "Single Plates",
      category: " ",
      description: "breaded chicken bites, French fries, French sauce",
      price: "6.50",
    },
    {
      name: "Cheddar Fries",
      type: "Single Plates",
      category: " ",
      description: "French fries with cheddar sauce & bacon",
      price: "3.80",
    },
    {
      name: "Hot Dog",
      type: "Single Plates",
      category: " ",
      description:
        "soft bun, Frankfurt sausage, ketchup, mustard, coleslaw, French fries",
      price: "6.00",
    },
    {
      name: "Open Special Bread",
      type: "Single Plates",
      category: " ",
      description:
        "soft bun, French fries, cheese, turkey shoulder, sausage, bacon, French sauce & mayonnaise",
      price: "7.00",
    },
    {
      name: "Ceasar's Salad",
      type: "Single Plates",
      category: " ",
      description:
        "lollo, chicken fillets, cherry tomatoes, croutons, graviera cheese, Ceasar's sauce",
      price: "6.00",
    },
    {
      name: "BAKAN Salad",
      type: "Single Plates",
      category: " ",
      description:
        "lollo, rocket, cherry tomatoes, super mix nuts, apple, graviera cheese, cucumber, croutons, balsamic",
      price: "6.50",
    },
    {
      name: "Cheese/Cold Cuts Platter",
      type: "Single Plates",
      category: " ",
      description:
        "bacon, Frankfurt sausage, shoulder, turkey, graviera cheese, edam cheese, breadsticks",
      price: "6.50",
    },
  ],
};
